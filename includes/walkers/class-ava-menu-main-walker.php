 ...
 * Walker class
 */
class Ava_Menu_Main_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$classes[] = 'ava-menu-item-' . $item->ID;

		if ( 0 < $depth ) {
			$classes[] = 'ava-sub-menu-item';
		}
		 ...
		 * @param int      $depth   Depth of menu item. Used for padding.
		 */
		$id = apply_filters( 'nav_menu_item_id', 'ava-menu-item-'. $item->ID, $item, $args, $depth );
		...
		if ( ! empty( $settings['menu_icon'] ) ) {
			$icon = ava_menu_tools()->get_icon_html( $settings['menu_icon'] );
		}

		if ( ! empty( $item->description ) ) {
			$desc = sprintf(
				'<div class="ava-menu-item-desc %2$s">%1$s</div>',
				$item->description,
				( 0 === $depth ) ? 'top-level-desc'  : 'sub-level-desc'
			);
		}

		$desc_allowed = ava_menu_option_page()->get_option( 'ava-show-top-menu-desc', 'true' );
		...
		$sub_desc_allowed = ava_menu_option_page()->get_option( 'ava-show-sub-menu-desc', 'true' );
		...
		if ( ! empty( $settings['menu_badge'] ) ) {
			$badge = ava_menu_tools()->get_badge_html( $settings['menu_badge'], $depth );
		}
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) || $this->is_mega_enabled( $item->ID ) ) {
			...
			$arrow_icon    = ava_menu_option_page()->get_option( 'ava-menu-' . $level . '-arrow', $default_arrow );
			$arrow         = ava_menu_tools()->get_dropdown_arrow_html( $arrow_icon );
		}

		$title = sprintf(
			'<div class="ava-menu-item-wrapper">%1$s<div class="ava-menu-title">%2$s%3$s</div>%4$s%5$s</div>',
			$icon,
			$title,
			$desc,
			$badge,
		...
		$mega_item = get_post_meta( $item->ID, ava_menu_post_type()->meta_key(), true );

		if ( $this->is_mega_enabled( $item->ID ) && ! $is_elementor ) {

			$content = '';

			do_action( 'ava-menu/mega-sub-menu/before-render', $item->ID );
			...
			do_action( 'ava-menu/mega-sub-menu/after-render', $item->ID );

			$item_output .= sprintf( '<div class="ava-sub-mega-menu">%s</div>', do_shortcode( $content ) );

		}

		ava_menu_tools()->add_menu_css( $item->ID, '.ava-menu-item-' . $item->ID );

		$item_output = apply_filters( 'ava-menu/main-walker/start-el', $item_output, $item, $this, $depth, $args );
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		$item_output = "</li>{$n}";
		$item_output = apply_filters( 'ava-menu/main-walker/end-el', $item_output, $item, $this, $depth, $args );
	...
	public function set_item_type( $item_id = 0, $depth = 0 ) {
		...
		wp_cache_set( 'item-type', $item_type, 'ava-menu' );
	...
	public function get_item_type() {
		return wp_cache_get( 'item-type', 'ava-menu' );
	}
	...
	public function is_mega_enabled( $item_id = 0 ) {
		...
		$menu_post     = ava_menu_post_type()->get_related_menu_post( $item_id );
	...
	public function get_settings( $item_id = 0 ) {
		...
		if ( null === $this->item_settings ) {
			$this->item_settings = ava_menu_settings_item()->get_settings( $item_id );
		}
